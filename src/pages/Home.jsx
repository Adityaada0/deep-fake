...
import { processFile } from "../assets/lotties/predictionLogic";

function Home() {
  const [isLoading, setIsLoading] = useState(false);
  const [file, setFile] = useState();
  const [fileType, setFileType] = useState(""); // To differentiate between image and video
  ...
  const handleChange = (e) => {
    ...
    if (uploadedFile) {
      ...
      processFile(fileName, setClassification, setConfidence, setIsLoading);
  ...
  const generateGraphData = (targetConfidence, isReal = true, numPoints = 15) => {
    ...
    for (let i = 0; i < numPoints; i++) {
      const variation = Math.random() * (10 - 0.5) + 0.5;
      const sharpTurn = Math.random() < 0.5 ? -1 : 1;
      ...
      data.push({
        elapsedTime: i * 100,
        confidence: Math.max(0, Math.min(value, 100)),
      });
    }
  ...
  useEffect(() => {
    if (confidence !== "") {
      ...
      const isReal = classification.toLowerCase() === "real";
  ...
  return (
    <>
      <Navbar />
      <div id="back" className={file ? "w-full h-[400px] pt-10" : "w-full h-[400px] pt-36"}>
        <div className="grid md:grid-cols-2 place-items-center my-20 mr-20 h-20 pt-20">
          <div className="flex flex-col gap-6 text-center md:text-left px-6">
            ...
            <div className="flex justify-center md:justify-start">
              ...
              >
                {ispopped && (
                  <div className="w-full h-[500px] scale-105 rounded-lg p-4 bg-[#398D8D] flex flex-col justify-center items-center overflow-auto">
                    <label
                    ...
                    {isLoading && (
                      <>
                        ...
                        <h1 className="text-white">
                          Processing your file... Please wait for 5 seconds.
                        </h1>
                      </>
                    )}
                  </div>
        ...
        <div className="px-24 mt-32 bg-white">
          <div className="flex flex-col justify-center items-center space-y-8 shadow-lg">
            ...
            <div className="flex flex-col justify-center items-center text-2xl">
              ...
              {confidence && (
                <p className="text-lg text-gray-600 m-0" >Confidence: {confidence}</p>
              )}
            </div>
          </div>
